...
static void _harness_t_sgxssl_call_apis(void) {
  auto ret = t_sgxssl_call_apis(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "t_sgxssl_call_apis returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_new_thread_func(void) {
  auto ret = new_thread_func(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "new_thread_func returned with error code: 0x%X\n", ret);
    abort();
  }
}
